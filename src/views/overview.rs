use chrono::{DateTime, Datelike, Local, Month, Months};
use egui::RichText;
...
impl Overview {
    pub fn new(storage: Box<dyn TimeStorage + Send>) -> Self {
        ...
        Self{
            end_date: now.with_day(now.date_naive().days_in_month()).expect("Could not get end day of month"),
    ...
    fn show(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame, ui: &mut egui::Ui) {
            ui.vertical_centered(|ui|{
                ui.horizontal_centered(|ui|{
                    ui.label("Start: ");
                    ui.label(RichText::new(format!("{}", self.start_date.to_rfc3339())));
                });
                ui.horizontal_centered(|ui|{
                    ui.label("End: ");
